...
describe("useEnrichedServices", () => {
  it("maps stay status to coffee emoji", () => {
    const { result } = renderHook(() =>
      useEnrichedServices([makeService("96", "stay")], FIXED_NOW)
    );
    expect(result.current[0].statusEmoji).toBe("☕");
    expect(result.current[0].statusLabel).toBe("Stay");
  });

  it("maps leave status to running emoji", () => {
    const { result } = renderHook(() =>
      useEnrichedServices([makeService("96", "leave")], FIXED_NOW)
    );
    expect(result.current[0].statusEmoji).toBe("🏃");
    expect(result.current[0].statusLabel).toBe("Leave");
  });

  it("maps gone status to wind emoji", () => {
    const { result } = renderHook(() =>
      useEnrichedServices([makeService("96", "gone")], FIXED_NOW)
    );
    expect(result.current[0].statusEmoji).toBe("💨");
    expect(result.current[0].statusLabel).toBe("Gone");
  });

  it("formats leaveAt as HH:mm display time", () => {
    const leaveAt = "2024-01-01T16:30:00+08:00";
    const { result } = renderHook(() =>
      useEnrichedServices([makeService("96", "leave", leaveAt)], FIXED_NOW)
    );
    expect(result.current[0].leaveAtDisplay).toMatch(/\d{2}:\d{2}/);
  });
